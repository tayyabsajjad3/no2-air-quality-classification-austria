...
| T3.9 Model deposit in TUWRD | Ready for team publication | The model file `outputs/models/no2_air_quality_classifier.joblib` was uploaded to the TUWRD draft together with generated outputs. The record is ready for review; final publication requires team/reviewer permission. |
...
| T3.10 Generated data deposit in TUWRD | Ready for team publication | The TUWRD draft contains `confusion_matrix.csv`, `confusion_matrix.png`, `model_metrics.json`, `test_predictions.csv`, and `no2_air_quality_classifier.joblib`. Reserved DOI: `https://doi.org/10.70124/4wqg-7oc34`. Final publication requires team/reviewer permission. |
...
1. Complete TUWRD publication/review for the draft record containing the generated outputs and trained model.
2. Export the final DMP PDF and maDMP JSON from the TU Wien DMP Tool.
3. Publish the final DMP/maDMP record if required by T4.4.
4. Complete the DMP comparison once the final DMP is available.
5. Create the final GitHub release after DOI/publication state is confirmed.
6. Complete the GitLab MR/PR if required by WP5 and if access is available.
