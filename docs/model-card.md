...
The training data is derived from EEA NO2 hourly measurement files stored in `data/raw/`. The current raw dataset contains 16 station-specific Parquet files and 140,160 hourly observations. The final version of this section must reference the DOI or persistent identifier for the training dataset once the DBRepo and repository deposits are complete.
