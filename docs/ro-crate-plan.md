...
The draft `ro-crate-metadata.json` is available in the repository root. The generated-output licence is recorded as CC BY 4.0 unless the verified EEA source-data licence requires a different compatible licence. The final RO-Crate cannot be completed until the group has the trained model deposit DOI, generated-data deposit DOI, DBRepo PID, and final artefact filenames. The current Zenodo DOI, DBRepo database identifier, and available ORCIDs are already included in the draft crate.
